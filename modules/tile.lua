...
function tile:create(xpos, ypos)
	local t = {}
	t = display.newGroup()
	t.rect = display.newRoundedRect(t, 10 + 50*xpos, 50*ypos, 45, 45, 2)
	t.xpos = xpos
	t.ypos = ypos
	t.val = math.random(-5, 5)
	t.text = display.newText(t, t.val, 10 + 50*xpos, 50*ypos, "media/Bungee-Regular.ttf")
...
function tile:touch(event)
	if event.target.selected == false and self.adjacentCheck(event.target) == true and event.target.disabled == false then
